...
export function buildComplianceReportPdf(
  ...
  const doc = new PDFDocument({
    ...
    info: {
      ...
      Author: "Nirikshan — Legal Metrology Assistant",
  ...
  doc
    .font("Helvetica")
    .fontSize(9)
    .text("Nirikshan — Legal Metrology Assistant  •  SIH26034", 56, 62, { width: pageWidth });
  doc
    .font("Helvetica-Bold")
    .fontSize(10)
  ...
  for (let i = 0; i < pageCount; i++) {
    ...
    const footerText = `Generated by Nirikshan on ${now.toISOString().replace("T", " ").slice(0, 19)} UTC  •  Page ${i + 1} of ${pageCount}`;
