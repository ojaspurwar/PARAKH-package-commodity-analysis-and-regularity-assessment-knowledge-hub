# 🎨 {{WEBSITE_NAME}} — UI/UX Design System & Frontend Architecture

> **Naming setup:** Edit only the values in the section below. Keep the placeholder names in the rest of this document unchanged so the same naming can be reused consistently across the website, mobile app, dashboard, reports, and browser title.

## ✏️ Edit Here Only — Website Identity

| Field | Value |
|---|---|
| Website / App name | `{{WEBSITE_NAME}}` |
| Short name | `{{WEBSITE_SHORT_NAME}}` |
| Project code | `{{PROJECT_CODE}}` |
| Organization / department | `{{ORGANIZATION_NAME}}` |
| Tagline | `{{WEBSITE_TAGLINE}}` |
| Logo alt text | `{{LOGO_ALT_TEXT}}` |
| Support contact | `{{SUPPORT_CONTACT}}` |
| Primary language | `English` |
| Secondary language | `हिन्दी` |
| Report title | `{{REPORT_TITLE}}` |

### Placeholder usage

- Browser title: `{{WEBSITE_NAME}}`
- Header and app logo: `{{WEBSITE_SHORT_NAME}}`
- Official logo label: `{{ORGANIZATION_NAME}} Logo`
- PDF report heading: `{{REPORT_TITLE}}`
- Welcome-screen tagline: `{{WEBSITE_TAGLINE}}`
- Accessibility label for the logo: `{{LOGO_ALT_TEXT}}`

> Replace each value above once, then use the resulting values everywhere in the implementation. Do not create separate copies of the website name for the mobile app, dashboard, or reports.

---

**Project:** `{{PROJECT_CODE}}` — `{{WEBSITE_NAME}}`  
...
1.  **Welcome & Language Selection (Splash Screen):**
    *   First-time open screen featuring the `{{ORGANIZATION_NAME}} Logo`.
